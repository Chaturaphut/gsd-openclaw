...
# Updated: 2026-04-05 — GSD v1.32.0: State Consistency & Autonomous Gates
...
- **Autonomous Gates:** หยุดงานอัตโนมัติเมื่อเจอความเสี่ยงหรือต้องการคนตัดสินใจ
...
## 🆕 Cherry-picked from GSD v1.32 (Added 2026-04-05)

### ⚖️ STATE.md Consistency Gates (from v1.32)
**ปัญหา:** ไฟล์ STATE.md อาจคลาดเคลื่อนจากความจริงบน disk (drift) เมื่อรัน parallel หรือหยุดกลางคัน
**วิธีใช้:** เพิ่มการตรวจสอบ Consistency:
- **State Validate:** ตรวจจับความต่างระหว่าง STATE.md และ filesystem ก่อนเริ่มงานใหม่
- **State Sync:** กู้คืน/สร้าง STATE.md ใหม่จากความจริงบน disk (ใช้ `--verify` สำหรับ dry-run)
- **Planned-Phase Recording:** บันทึก state ทันทีหลังจบ stage Planning เพื่อป้องกันการรันซ้ำซ้อน

### 🛡️ Research Gate Blocks Planning (from v1.32)
**ปัญหา:** Planner พยายามวางแผนทั้งที่มีคำถามสำคัญ (Open Questions) ที่ยังไม่ได้คำตอบจาก Research
**กฎ:** ห้ามเริ่ม Stage 3 (Plan) หากยังมี unresolved open questions ใน RESEARCH.md ที่ส่งผลต่อการออกแบบ
- MumuX ต้องตรวจสอบ RESEARCH.md และเคลียร์ blocker ก่อนอนุญาตให้ Planner ทำงาน

### ⏱️ Sequential Worktree Dispatch (from v1.32)
**ปัญหา:** การรัน parallel (Wave execution) โดยใช้ timing-based stagger อาจทำให้เกิด race condition
**วิธีใช้:** เปลี่ยนจากการรอเวลา (stagger) เป็น sequential task dispatch:
- ใช้ระบบ Task queue เพื่อรันงานใน background ทีละส่วนอย่างมั่นใจว่า environment พร้อม

---

## 🆕 Cherry-picked from GSD v1.31 (Added 2026-04-03)

### 🔒 Security-First Enforcement Layer (from v1.31)
**ปัญหา:** AI อาจเสนอโค้ดที่มีช่องโหว่ หรือละเลย security constraints ที่ระบุไว้ใน PROJECT.md
**วิธีใช้:** เพิ่ม stage **Security-Enforcement** หลัง Execute-phase:
- รัน threat-model-anchored verification เพื่อตรวจหา security flaws ตาม context ของ project
- ตรวจสอบว่าทุก security requirement ใน PROJECT.md ถูกทำตาม
- สร้าง `SECURITY-ENFORCEMENT.md` เพื่อบันทึกผลการตรวจสอบ

### 🤖 Headless Prompt Overhaul (from v1.31)
**ปัญหา:** การโต้ตอบกับ AI ในงานที่เป็น batch หรือ automation มี friction สูง
**วิธีใช้:** รองรับการทำงานแบบ Headless เต็มรูปแบบ:
- SDK สามารถขับเคลื่อน full lifecycle ของ project โดยไม่ต้องมี interactive patterns
- ใช้ `--auto` flag ร่วมกับ headless prompts เพื่อรันงานจนจบ (ถ้าไม่มี blocker)

### 📝 Verified Documentation Generation (from v1.31)
**ปัญหา:** Documentation มักจะไม่อัปเดตตามโค้ดที่เปลี่ยนไป
**วิธีใช้:** ใช้คำสั่ง `/gsd:docs-update` เพื่อรัน workflow การอัปเดต doc โดยเฉพาะ:
- ตรวจสอบความสอดคล้องระหว่าง code และ documentation
- อัปเดต doc files (เช่น README.md, docs/*.md) ให้ตรงกับ implementation ล่าสุด
- บังคับใช้มาตรฐานการเขียน doc ตาม config

### 🏗️ Project Code Prefixing (from v1.31)
**ปัญหา:** ใน monorepo หรือ workspace ใหญ่ๆ การแยก phase directory ทำได้ยาก
**วิธีใช้:** ตั้งค่า `project_code` ใน `config.json` เพื่อ prefix phase directories:
- ตัวอย่าง: `project_code: "API"` → `.planning/phases/API-phase-1-setup/`
- ช่วยให้แยกแยะงานจากคนละ project ใน workspace เดียวกันได้ชัดเจน

### 🌊 Single-Phase Parallel Execution (from v1.31)
**ปัญหา:** ต้องการเร่งงานเฉพาะ phase เดียวให้เร็วที่สุด
**วิธีใช้:** ใช้ `--only N` flag ใน execute-phase เพื่อรันเฉพาะ phase N แบบ parallel เต็มกำลัง

### 🔗 Chain Mode (from v1.31)
**ปัญหา:** ต้องสลับคำสั่ง /gsd:discuss → /gsd:plan → /gsd:execute บ่อยๆ
**วิธีใช้:** ใช้ `--chain` flag ใน discuss-phase เพื่อให้ AI รัน plan และ execute ต่อทันทีหลังจากตอบคำถามครบ

---

## 🆕 Cherry-picked from GSD v1.26–v1.30 (Updated 2026-03-29)
...
**ปัญหา:** Dev สร้าง function เเปล่า / TODO / placeholder แล้วบอก "เสร็จแล้ว"
**วิธีใช้:** QA ต้อง grep หา stubs ก่อนรับงาน:
  ```bash
...
**Last checked:** 2026-04-05 | **Upstream version:** v1.32.0 (Autonomous Gates)
**Cron:** Zen (Agent #60) เช็คทุกสัปดาห์ รายงาน CEO ถ้ามี feature ใหม่ที่น่าใช้

---

*Effective: 2026-03-22 | Version: 1.9 (updated 2026-04-05)*
